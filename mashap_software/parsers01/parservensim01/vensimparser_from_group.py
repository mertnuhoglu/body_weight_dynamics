...
#parses equations that are exported as a group from vensim:
# 
...
def convert(file='model.mdl', sort=True):
    text = open(file).read()
    text = format_vensim_eqns(text)
    (eqns, eqns_stocks, stock_inits) = extract_stocks(text)
    if sort:
       eqns.sort()
    open( file.split('.')[0] + '_latex.txt','wb').write('\n'.join(eqns))
    open('eqns_stocks_latex.txt','wb').write('\n'.join(eqns_stocks))
    open('stock_inits_latex.txt','wb').write('\n'.join(stock_inits))

def format_normal_eqns(text):
    return format_for_latex(
            space_to_underscore((text))).strip()
def format_vensim_eqns(text):
    return convert_lookups(
            ...
            one_line_per_eqn(
            trim_from_units_to_stars(
            trim_utf8_notation(text))))))))))).strip()
def put_verb_commands(text):
    p = re.compile(r'(\b(?!\d)\w+\b)')
    return p.subn(r'\\verb|\1|',text)[0]
...
def trim_from_units_to_stars(text):
# used for trimming equations in docs of vensim not normal model file
    p = re.compile(r'(?s)Units:.*?\*{10,}')
    return p.subn('',text)[0]
