...
def extract_stocks(text):
    ...
    eqns_stocks = []
    eqns = []
    pattern_whole_line = r'^(?P<eqn>.+?)~(?P<unit>.+?)~(?P<description>.+?)\|$'
    c_whole_line = re.compile(pattern_whole_line)
    pattern_eqn = r'^(?P<var_name>.+?)=(?P<formula>.+?)'
    c_eqn = re.compile(pattern_eqn)
    ...
    for line in text.splitlines():
        m_whole_line = c_whole_line.search(line)
        if m_whole_line:
            eqn = m_whole_line.group('eqn').strip()
            unit = m_whole_line.group('unit').strip()
            description = m_whole_line.group('description').replace('_',' ').strip()
        else: # for lines added during conversion
            eqn = line
            unit = ''
            description = ''
        ...
        if m:
            ...
            var_name = m.group('stock').strip()
            formula = m.group('formula').strip()
            stock_mappings[var_name] = Bunch(var_name = var_name, formula = formula, unit = unit,
                                             description = description, init = m.group('init'))
        else:
            eqns.append(eqn)
            m_eqn = c_eqn.search(eqn)
            if m_eqn:
                var_name = m_eqn.group('var_name').strip()
                formula = m_eqn.group('formula').strip()
            else:
                var_name = eqn
                formula = ''
            var_mappings[var_name] = Bunch(var_name = var_name, formula = formula, unit = unit, description = description)
    mappings_bunch = Bunch(var_mappings = var_mappings, stock_mappings = stock_mappings)
    return (eqns, eqns_stocks, stock_inits, mappings_bunch)

def convert(file='model.mdl', text=''):
    if not text:
        text = open(file).read()
    text = format_vensim_eqns(text)
    (eqns, eqns_stocks, stock_inits, mappings) = extract_stocks(text)
    eqns.sort()
    return ('\n'.join(eqns), '\n'.join(eqns_stocks), '\n'.join(stock_inits), mappings)

def write_files(eqns, eqns_stocks, stock_inits):
